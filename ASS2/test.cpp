...
tree_node *find_root(tree_node *node);
     ...
     */
        ...
        // // Part 1 - case 5
        {
            ...
            try
            {
                if (father->l_child->data != 1 || father->r_child->data != 3)
                    throw e;
                if (!judge_bst(bst, root_tree_count) || root_tree_count != bst->root->tree_count)
                    throw e;
                count++;
            }
            catch (assign2_exception::exception)
        ...
        */
        // Part 4 - case 1
        {
            ...
            try
            {
                ...
                if (*target_node != NULL)
                    e = 1, throw e; // judgement step 1.
                count++;
                find_in_BST(bst, 50, target_node);
                if (*target_node == NULL || (*target_node)->data != 50)
                    delete_tree(bst->root), delete bst, e = 2, throw e; // judgement step 2.
                count++;
                find_in_BST(bst, 55, target_node);
                if (*target_node == NULL || (*target_node)->data != 55)
                    delete_tree(bst->root), delete bst, e = 3, throw e; // judgement step 3.
                count++;
                find_in_BST(bst, 90, target_node);
                if (*target_node == NULL || (*target_node)->data != 90)
                    delete_tree(bst->root), delete bst, e = 4, throw e; // judgement step 4.
                count++;
                find_in_BST(bst, 61, target_node);
                if (*target_node == NULL || (*target_node)->data != 61)
                    delete_tree(bst->root), delete bst, e = 5, throw e; // judgement step 5.
                count++;
                find_in_BST(bst, 29, target_node);
                if (*target_node == NULL || (*target_node)->data != 29)
                    delete_tree(bst->root), delete bst, e = 6, throw e; // judgement step 6.
                count++;
                find_in_BST(bst, 36, target_node);
                if (*target_node == NULL || (*target_node)->data != 36)
                    delete_tree(bst->root), delete bst, e = 7, throw e; // judgement step 7.
                count++;
                find_in_BST(bst, 32, target_node);
                if (*target_node == NULL || (*target_node)->data != 32)
                    delete_tree(bst->root), delete bst, e = 8, throw e; // judgement step 8.
                count++;
                delete_tree(bst->root);
                delete bst;
            }
        ...
        */
        ...
        // Part 5 - case 1
        {
            ...
            try
            {
                ...
                int tree_c = 0;
                if (!judge_bst(bst, tree_c) || bst->root != test_node_one)
                    delete_tree(bst->root), delete bst, throw e = 1;
                count++;
                std::cout << "------------------\n";
                splay(bst, test_node_two);
                tree_c = 0;
                if (!judge_bst(bst, tree_c) || bst->root != test_node_two)
                    delete_tree(bst->root), delete bst, throw e = 2;
                count++;
                splay(bst, test_node_three);
                tree_c = 0;
                if (!judge_bst(bst, tree_c) || bst->root != test_node_three)
                    delete_tree(bst->root), delete bst, throw e = 3;
                count++;
                delete_tree(bst->root);
                delete bst;
            }
        ...
        // Part 5 - case 2 //To test the zig and zag
        {
            ...
            try
            {
                ...
                if (bst->root != father->father || bst->root->r_child != father)
                    delete_tree(bst->root), delete bst, throw e;
                count++;
        ...
        // Part 5 - case 3 //To test the zig and zag
        {
            ...
            try
            {
                ...
                if (bst->root != father->father || bst->root->l_child != father)
                    delete_tree(bst->root), delete bst, throw e;
                count++;
                delete_tree(bst->root);
                delete bst;
    ...
    bool judge_bst(BST *bst, int &tree_c)
    {
        if (bst->root != NULL)
        {
            ...
            if (bst->root->l_child != NULL)
            {
                if (bst->comp(bst->root->data, bst->root->l_child->data) < 0)
                    return false;
                BST *new_bst = new BST;
                new_bst->comp = bst->comp;
                new_bst->root = bst->root->l_child;
            ...
            if (bst->root->r_child != NULL)
            {
                if (bst->comp(bst->root->data, bst->root->r_child->data) > 0)
                    return false;
                BST *new_bst = new BST;
                new_bst->comp = bst->comp;
                new_bst->root = bst->root->r_child;
